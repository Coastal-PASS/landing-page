 ...
 */
export const SystemsSection = ({ implementNote, options, connectivity }: SystemsSectionProps): ReactElement | null => {
    ...
    <section className="bg-white py-25">
      <div className="mx-auto max-w-6xl space-y-10 px-4">
        <div className="flex flex-col gap-6 rounded-3xl border border-surface-muted/80 bg-surface-muted/40 p-6 shadow-card lg:flex-row lg:items-center">
          ...
          />
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-brand-primary">Implement Base Kit</p>
            <p className="mt-2 text-brand-body">{implementNote}</p>
          </div>
        </div>

        <div className="text-center">
          <p className="text-sm font-semibold uppercase tracking-wide text-brand-primary">Tractor control options</p>
          <h2 className="mt-2 text-3xl font-semibold text-brand-heading">Choose the in-cab experience that fits your crew</h2>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          {(options ?? []).map((option) => (
            <article key={option.title} className="h-full rounded-3xl border border-surface-muted/80 bg-white p-6 shadow-card">
              ...
              <h3 className="text-lg font-semibold text-brand-heading">{option.title}</h3>
              <p className="mt-2 text-sm text-brand-body">{option.description}</p>
            </article>
          ))}
        </div>
        ...
        {connectivity ? (
          <div className="flex flex-col gap-6 rounded-3xl border border-surface-muted/80 bg-white p-6 shadow-card lg:flex-row lg:items-center">
            <div className="flex-1">
              <h3 className="text-2xl font-semibold text-brand-heading">{connectivity.title}</h3>
              <p className="mt-2 text-brand-body">{connectivity.description}</p>
            </div>
            {connectivity.icon ? (
              <Image src={connectivity.icon} alt="Connectivity hardware" width={220} height={180} className="mx-auto lg:mx-0" />
            ) : null}
          </div>
        ) : null}
      </div>
