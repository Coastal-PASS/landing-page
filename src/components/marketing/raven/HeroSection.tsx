 ...
 */
export const HeroSection = ({ hero, logos }: HeroSectionProps): ReactElement | null => {
  if (!hero) {
    return null;
  }
  ...
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-brand-heading via-brand-primary to-brand-violet py-25 text-white">
      <div className="absolute inset-0 bg-[url('/assets/img/ct/hero.jpg')] bg-cover bg-center opacity-10" aria-hidden />
      <div className="relative mx-auto flex max-w-6xl flex-col gap-10 px-4 lg:flex-row lg:items-center">
        <div className="flex-1 space-y-4">
          ...
          <h1 className="text-4xl font-semibold leading-tight text-white lg:text-5xl">{hero.title}</h1>
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            {safeLogos.map((logo) => (
              <div key={logo.alt} className="rounded-xl border border-white/20 bg-white/90 px-4 py-2">
