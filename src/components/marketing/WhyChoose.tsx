  ...
  <section className="py-30">
    <div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-2 lg:items-center">
      <div className="space-y-6">
        <p className="text-sm font-semibold uppercase tracking-wide text-brand-primary">Why choose us</p>
        <h2 className="text-3xl font-semibold text-brand-heading">High-tech advantage with old fashioned service</h2>
        <p className="text-brand-body">
          Our team pairs boots-on-the-ground experience with a modern provider stack, so growers get proactive support, actionable data, and equipment that simply works.
        </p>
        <dl className="space-y-5">
          {reasons.map((reason, index) => (
            <div key={reason.title} className="flex gap-4 rounded-2xl border border-surface-muted/70 bg-white/80 p-5 shadow-card">
              <dt className="text-3xl font-semibold text-brand-primary">{String(index + 1).padStart(2, '0')}</dt>
              <dd>
                <p className="text-lg font-semibold text-brand-heading">{reason.title}</p>
      ...
      <div className="relative">
        <div className="hidden lg:block">
          <div className="absolute -top-6 -right-6 h-40 w-40 rounded-full bg-brand-accent/20 blur-3xl" aria-hidden />
        </div>
        <Image
